...
import { iGetImageModeStateApiResponse } from '@/types/iMultimedia'
...
const getparam=multimedia.imagemodestate.param.get
const setparam=multimedia.imagemodestate.param.set
const keyState=multimedia.imagemodestate.key.state

const fetchCameraState = async(param: string,): Promise<iGetImageModeStateApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetImageModeStateApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetImageModeStateApiResponse, Error>({
            ...
            <>
                <div className="flex gap-2">
                    <Label htmlFor="name" className="text-left min-w-40">
                        ...
                        />
                        <Button onClick={
                            () => {updateCameraState(setparam, keyState, state)}
                        }>Submit</Button>
                </div>
            </>
    )
...
// const cmd='http://'+login.url+':'+login.port+'/param.cgi?cmd='
// const auth='&user='+login.user+'&pwd='+login.password
// const param=multimedia.imagemodestate

// const fetchData = async(param: string): Promise<iGetImageModeStateApiResponse> => {
...
//         data,
//         isSuccess
//     }: UseQueryResult<iGetImageModeStateApiResponse, Error> = useQuery<iGetImageModeStateApiResponse, Error>({
//         queryKey: ['imagestateparams', { param }],
...
//                         Image State
//                     </Label>
//                     <Input id="name" value={data.state} className="col-span-3" />
//                 </div>
//             </>
//     )
