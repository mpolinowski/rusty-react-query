...
const getparam=features.ftpactionattr.param.get
const setparam=features.ftpactionattr.param.set
const keysnapcount=features.ftpactionattr.key.snapcount

const fetchCameraState = async(param: string,): Promise<iGetFtpActionAttrApiResponse> => {
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetFtpActionAttrApiResponse, Error>({
        queryKey: ['custshedaudioparams', { getparam }],
            ...
            <>
                <div className="flex gap-2">
                    <Label htmlFor="name" className="text-left min-w-40">
                        FTP Count of Snapshots
                    </Label>
                        <Input
                            defaultValue={response?.snapcount}
                            onChange={(e) => setSnapcount(e.target.value)}
                            className="col-span-3"
                        />
                        <Button onClick={
                            () => {updateCameraState(setparam, keysnapcount, snapcount)}
                        }>Submit</Button>
                </div>
            </>
    )
...
// const cmd='http://'+login.url+':'+login.port+'/param.cgi?cmd='
// const auth='&user='+login.user+'&pwd='+login.password
// const param=features.ftpaction
...
//         queryKey: ['ftpactionparams', { param }],
