...
const getparam=features.wizardstatus.param.get
const setparam=features.wizardstatus.param.set
const keystatus=features.wizardstatus.key.status

const fetchCameraState = async(param: string,): Promise<iGetWizardStatusApiResponse> => {
...
export default function DataRequest(): React.JSX.Element {
    const [wzstatus, setWzstatus] = useState('')
    const {
        isLoading,
        isError,
        error,
        data: response
    } = useQuery<iGetWizardStatusApiResponse, Error>({
        queryKey: ['wizardstatus', { getparam }],
                ...
                <div className="flex gap-2">
                    <Label htmlFor="name" className="text-left min-w-40">
                        ...
                        <Input
                            ...
                            onChange={(e) => setWzstatus(e.target.value)}
                            className="col-span-3"
                        />
                        <Button onClick={
                            () => {updateCameraState(setparam, keystatus, wzstatus)}
                        }>Submit</Button>
                </div>
            </>
    )
