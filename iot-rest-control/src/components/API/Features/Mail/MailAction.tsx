...
import { iGetMailActionApiResponse } from '@/types/iFeatures'
...
const getparam=features.mailaction.param.get
const setparam=features.mailaction.param.set
const keyenable=features.mailaction.key.enable
const keyinterval=features.mailaction.key.interval

const fetchCameraState = async(param: string,): Promise<iGetMailActionApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetMailActionApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetMailActionApiResponse, Error>({
        queryKey: ['mailparams', { getparam }],
...
// const cmd='http://'+login.url+':'+login.port+'/param.cgi?cmd='
// const auth='&user='+login.user+'&pwd='+login.password
// const param=features.mailaction

// const fetchData = async(param: string): Promise<iGetMailActionApiResponse> => {
...
//         data,
//         isSuccess
//     }: UseQueryResult<iGetMailActionApiResponse, Error> = useQuery<iGetMailActionApiResponse, Error>({
//         queryKey: ['mailparams', { param }],
