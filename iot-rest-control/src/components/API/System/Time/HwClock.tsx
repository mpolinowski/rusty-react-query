...
import { iGetHwClockApiResponse } from '@/types/iSystem'
...
const getparam=system.hwstate.param.get
const setparam=system.hwstate.param.set
const keyhwclock=system.hwstate.key.hwclock

const fetchCameraState = async(param: string,): Promise<iGetHwClockApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetHwClockApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    const [hwclock, setHwclock] = useState('')
    const {
        isLoading,
        isError,
        error,
        data: response
    } = useQuery<iGetHwClockApiResponse, Error>({
        queryKey: ['timeparams', { getparam }],
            ...
            <>
                <div className="flex gap-2 mb-3">
                    <Label htmlFor="name" className="text-left min-w-40">
                        ...
                        <Input
                            defaultValue={response?.hwclock}
                            onChange={(e) => setHwclock(e.target.value)}
                            className="col-span-3"
                        />
                        <Button onClick={
                            () => {updateCameraState(setparam, keyhwclock, hwclock)}
