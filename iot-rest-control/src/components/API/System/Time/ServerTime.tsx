...
import { iGetServerTimeApiResponse } from '@/types/iSystem'
...
const getparam=system.servertime.param.get
const setparam=system.servertime.param.set
const keytimezone=system.servertime.key.timezone
const keytime=system.servertime.key.time

const fetchCameraState = async(param: string,): Promise<iGetServerTimeApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetServerTimeApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetServerTimeApiResponse, Error>({
            ...
            <>
                <div className="flex gap-2 mb-3">
                    <Label htmlFor="name" className="text-left min-w-40">
                        ...
                        <Input
                            defaultValue={response?.timezone}
                            onChange={(e) => setTimezone(e.target.value)}
                            className="col-span-3"
                        />
                        <Button onClick={
                            () => {updateCameraState(setparam, keytimezone, timezone)}
                        }>Submit</Button>
                </div>
                <div className="flex gap-2 mb-3">
                    <Label htmlFor="name" className="text-left min-w-40">
