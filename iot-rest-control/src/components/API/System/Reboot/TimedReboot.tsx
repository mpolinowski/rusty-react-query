...
import { iGetTimerRebootApiResponse } from '@/types/iSystem'
...
const getparam=system.timerreboot.param.get
const setparam=system.timerreboot.param.set
const keyenable=system.timerreboot.key.enable
const keyday=system.timerreboot.key.day
const keytime=system.timerreboot.key.time

const fetchCameraState = async(param: string,): Promise<iGetTimerRebootApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetTimerRebootApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetTimerRebootApiResponse, Error>({
        queryKey: ['autorebootparams', { getparam }],
            ...
            <>
                <div className="flex gap-2 mb-3">
                    <Label htmlFor="name" className="text-left min-w-40">
                        ...
                        <Input
                            defaultValue={response?.sr_enable}
