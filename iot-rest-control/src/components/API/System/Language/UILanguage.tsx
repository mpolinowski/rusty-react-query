...
import { iGetLanguageApiResponse } from '@/types/iSystem'
...
const getparam=system.language.param.get
const setparam=system.language.param.set
const keylancode=system.language.key.lancode

const fetchCameraState = async(param: string,): Promise<iGetLanguageApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetLanguageApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetLanguageApiResponse, Error>({
