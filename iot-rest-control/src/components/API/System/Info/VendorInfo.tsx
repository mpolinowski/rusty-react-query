...
import { iGetVendorInfoApiResponse } from '@/types/iSystem'
...
const getparam=system.vendorinfo.param.get
const setparam=system.vendorinfo.param.set
const keyproduct=system.vendorinfo.key.product
const keyseries=system.vendorinfo.key.series
const keyvendor=system.vendorinfo.key.vendor

const fetchCameraState = async(param: string,): Promise<iGetVendorInfoApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetVendorInfoApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetVendorInfoApiResponse, Error>({
            ...
            <>
                ...
                <div className="flex gap-2 mb-3">
                    <Label htmlFor="name" className="text-left min-w-40">
                        ...
                        <Input
                            defaultValue={response?.series}
                            onChange={(e) => setSeries(e.target.value)}
                            className="col-span-3"
                        />
                        <Button onClick={
                            () => {updateCameraState(setparam, keyseries, series)}
                        } disabled>Submit</Button>
                </div>
                <div className="flex gap-2 mb-3">
                    <Label htmlFor="name" className="text-left min-w-40">
                        ...
                        <Input
                            defaultValue={response?.vendor}
                            onChange={(e) => setVendor(e.target.value)}
                            className="col-span-3"
                        />
                        <Button onClick={
                            () => {updateCameraState(setparam, keyvendor, vendor)}
                        } disabled>Submit</Button>
                </div>
            </>
    )
