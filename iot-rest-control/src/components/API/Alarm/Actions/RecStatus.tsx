...
import { iGetRecStatusApiResponse } from '@/types/iAlarm'
...
const getparam=alarm.recordstatus.param.get
const setparam=alarm.recordstatus.param.set
const keyrecstatus=alarm.recordstatus.key.recstatus

const fetchCameraState = async(param: string,): Promise<iGetRecStatusApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetRecStatusApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetRecStatusApiResponse, Error>({
        queryKey: ['recstatusparams', { getparam }],
            ...
            <>
                <div className="flex gap-2 mb-3">
                    <Label htmlFor="name" className="text-left min-w-40">
                        ...
                        <Input
                            defaultValue={response?.recstatus}
                            onChange={(e) => setRecstatus(e.target.value)}
                            className="col-span-3"
                        />
                        <Button onClick={
                            () => {updateCameraState(setparam, keyrecstatus, recstatus)}
                        }>Submit</Button>
                </div>
            </>
    )
...
// const cmd='http://'+login.url+':'+login.port+'/param.cgi?cmd='
// const auth='&user='+login.user+'&pwd='+login.password
// const param=alarm.recordstatus

// const fetchData = async(param: string): Promise<iGetRecStatusApiResponse> => {
...
//         data,
//         isSuccess
//     }: UseQueryResult<iGetRecStatusApiResponse, Error> = useQuery<iGetRecStatusApiResponse, Error>({
//         queryKey: ['recstatusparams', { param }],
...
//                         Recording Status
//                     </Label>
//                     <Input id="name" value={data.recstatus} className="col-span-3" />
//                 </div>
//             </div>
//     )
