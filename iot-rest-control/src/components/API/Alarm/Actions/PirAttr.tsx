...
import { iGetPirAttrApiResponse } from '@/types/iAlarm'
...
const getparam=alarm.pirattr.param.get
const setparam=alarm.pirattr.param.set
const keyenable=alarm.pirattr.key.enable

const fetchCameraState = async(param: string,): Promise<iGetPirAttrApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetPirAttrApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetPirAttrApiResponse, Error>({
        queryKey: ['pirparams', { getparam }],
...
// const cmd='http://'+login.url+':'+login.port+'/param.cgi?cmd='
// const auth='&user='+login.user+'&pwd='+login.password
// const param=alarm.pirattr

// const fetchData = async(param: string): Promise<iGetPirAttrApiResponse> => {
...
//         data,
//         isSuccess
//     }: UseQueryResult<iGetPirAttrApiResponse, Error> = useQuery<iGetPirAttrApiResponse, Error>({
//         queryKey: ['pirparams', { param }],
