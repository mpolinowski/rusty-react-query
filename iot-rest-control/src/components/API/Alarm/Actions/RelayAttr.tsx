...
import { iGetOutputAttrApiResponse } from '@/types/iAlarm'
...
const getparam=alarm.relayaction.param.get
const setparam=alarm.relayaction.param.set
const keyenable=alarm.relayaction.key.enable
const keyidle=alarm.relayaction.key.idle
const keyinterval=alarm.relayaction.key.interval

const fetchCameraState = async(param: string,): Promise<iGetOutputAttrApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetOutputAttrApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetOutputAttrApiResponse, Error>({
        queryKey: ['relayparams', { getparam }],
...
// const cmd='http://'+login.url+':'+login.port+'/param.cgi?cmd='
// const auth='&user='+login.user+'&pwd='+login.password
// const param=alarm.relayaction

// const fetchData = async(param: string): Promise<iGetOutputAttrApiResponse> => {
...
//         data,
//         isSuccess
//     }: UseQueryResult<iGetOutputAttrApiResponse, Error> = useQuery<iGetOutputAttrApiResponse, Error>({
//         queryKey: ['relayparams', { param }],
