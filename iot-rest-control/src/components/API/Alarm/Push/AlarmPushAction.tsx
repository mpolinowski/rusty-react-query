...
import { iGetPushActionApiResponse } from '@/types/iAlarm'
...
const getparam=alarm.pushaction.param.get
const setparam=alarm.pushaction.param.set
const keyenable=alarm.pushaction.key.enable
const keyinterval=alarm.pushaction.key.interval

const fetchCameraState = async(param: string,): Promise<iGetPushActionApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetPushActionApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetPushActionApiResponse, Error>({
        queryKey: ['pushparams', { getparam }],
...
// const cmd='http://'+login.url+':'+login.port+'/param.cgi?cmd='
// const auth='&user='+login.user+'&pwd='+login.password
// const param=alarm.pushaction

// const fetchData = async(param: string): Promise<iGetPushActionApiResponse> => {
...
//         data,
//         isSuccess
//     }: UseQueryResult<iGetPushActionApiResponse, Error> = useQuery<iGetPushActionApiResponse, Error>({
//         queryKey: ['pushparams', { param }],
