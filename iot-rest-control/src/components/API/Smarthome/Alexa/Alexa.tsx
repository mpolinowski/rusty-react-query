...
import { iGetAlexaApiResponse } from '@/types/iSmarthome'
...
const getparam=smarthome.alexa.param.get
const setparam=smarthome.alexa.param.set
const keyenable=smarthome.alexa.key.enable

const fetchCameraState = async(param: string,): Promise<iGetAlexaApiResponse> => {
    ...
    if (response.ok) {
        ...
        const jsonData: iGetAlexaApiResponse = JSON.parse(cleanedTextResponse)
...
export default function DataRequest(): React.JSX.Element {
    ...
    const {
        ...
        error,
        data: response
    } = useQuery<iGetAlexaApiResponse, Error>({
            ...
            <>
                <div className="flex gap-2 mb-3">
                    <Label htmlFor="name" className="text-left min-w-40">
                        ...
                        <Input
                            defaultValue={response?.alexa_enable}
...
// const cmd='http://'+login.url+':'+login.port+'/param.cgi?cmd='
// const auth='&user='+login.user+'&pwd='+login.password
// const param=smarthome.alexa

// const fetchData = async(param: string): Promise<iGetAlexaApiResponse> => {
...
//         data,
//         isSuccess
//     }: UseQueryResult<iGetAlexaApiResponse, Error> = useQuery<iGetAlexaApiResponse, Error>({
//         queryKey: ['alexaparams', { param }],
...
//                     </Label>
//                     <Input id="name" value={data.alexa_enable} className="col-span-3" />
//                 </div>
//             </div>
//             <SheetFooter>
//                 <SheetClose asChild>
